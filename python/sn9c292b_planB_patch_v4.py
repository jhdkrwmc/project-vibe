#!/usr/bin/env python3
"""
SN9C292B Plan B Patcher (v4, final) — correct LE word-sum checksum @ tail + optional --recalc-only

Fixes from v3:
- Uses the correct LE 16-bit word-sum across the image. The compensator is written to the *last two bytes*.
- Adds --recalc-only yes to only recompute & write the footer for an already patched image.
...
def parse_int(x): return int(str(x), 0)
